...
/datum/candidate_poll
	...
	var/list/obj/abstract/screen/alert/poll_alert/alert_buttons = list()
...
/datum/candidate_poll/New(polled_role, polled_question, poll_duration, poll_jumpable)
	role = polled_role
...
/datum/candidate_poll/Destroy()
	if(src in SSpolling.currently_polling)
		...
		return// the above proc will call a qdel in 0.5 SECONDS
	jump_to_me = null
	signed_up = null
	return ..()
...
/datum/candidate_poll/proc/sign_up(mob/candidate, silent = FALSE)
	...
	for(var/obj/abstract/screen/alert/poll_alert/linked_button as anything in alert_buttons)
...
/datum/candidate_poll/proc/do_never_for_this_round(mob/candidate)
	candidate.client.prefs.roles[role] = ROLEPREF_NO
...
/datum/candidate_poll/proc/undo_never_for_this_round(mob/candidate)
	candidate.client.prefs.roles[role] = ROLEPREF_YES
